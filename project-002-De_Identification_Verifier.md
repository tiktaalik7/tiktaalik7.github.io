...
+ 타겟 파일 정보에 따라 View를 계속해서 Update해야 하므로 MVC 패턴을 채용한다.
...
+ 알고리즘  
사진의 가로 세로 비율이 사진이 그려질 배경의 가로 세로 비율과 비교해 긴 부분(가로 또는 세로)을 기준으로 사진과 배경의 비율을 구한다. 이후 해당 비율을 사진 전체에 적용하는 방식을 사용해 다양한 규격의 이미지를 비식별화 검증 도구에서 일관되게 확인할 수 있도록 함
...
## 데이터 로딩 시간 개선
+ 이번 프로젝트에서 사용한 타겟 파일은 $4000x4000로 로딩 시간이 오래 걸림  
...
크라우드 워커 분들 중 프로그램의 실행환경을 옮겨다니며 작업하는 분이 계셨다. 따라서 상대 경로를 사용하도록 프로그램을 변경하였다.

+ 절대 경로  
  + 장점 : 사용자가 원하는 디렉토리에 타겟파일과 프로그램을 관리할 수 있다.
  + 단점 : 한 번 설정해둔 디렉토리에서 벗어날 수 없다.
  
+ 상대 경로  
  + 장점 : 프로그램의 작업 환경을 바꿔가며 사용할 수 있다.
...
+ 문제 발생 및 대처  
1. 프로그램에 사용한 글꼴이 사용자의 컴퓨터에 존재하지 않아 발생하는 문제  
    + Windows의 기본글꼴이 사용되게끔 글꼴설정을 제외하고 배포
2. 프로그램을 다양한 실행환경에서 옮겨다니며 실행할 때 경로 변경으로 인해 발생하는 문제  
    + 사용자의 절대경로를 이용하지 않고 상대경로로 동작하게 수정
3. 타겟 파일의 오류로 인해 프로그램이 정지하는 문제
